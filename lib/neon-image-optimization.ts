import { neonPool } from "./neon-client"
...
export class NeonImageOptimizationService {
  ...
  async getPropertyImages(propertyId: number): Promise<any[]> {
    const result = await neonPool.executeQuery(
      `
      SELECT * FROM property_images_neon 
      WHERE property_id = $1 AND processing_status = 'completed'
  ...
  async processPendingImages(batchSize = 10): Promise<{
    ...
    // Get pending images
    const pendingImages = await neonPool.executeQuery(
      `
    ...
    for (const { property_id } of pendingImages) {
      try {
        ...
        const propertyImages = await neonPool.executeQuery(
          `
          SELECT * FROM property_images_neon 
          WHERE property_id = $1 AND processing_status = 'pending'
          ORDER BY display_order
        `,
          [property_id],
        )
    ...
    // Get remaining count
    const remainingResult = await neonPool.executeQuery(`
  ...
  async getProcessingStats(): Promise<any> {
    const stats = await neonPool.executeQuery(`
      SELECT 
        processing_status,
        COUNT(*) as count,
