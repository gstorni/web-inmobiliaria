...
export class SecureTokkoClient {
  ...
  constructor(config: Omit<SecureTokkoConfig, "readOnlyMode">) {
    this.config = { ...config, readOnlyMode: true }
    this.security = TokkoSecurityManager.getInstance()

    // Only validate API key when actually making requests, not during construction
  }

  private validateApiKey() {
    if (!this.config.apiKey) {
      throw new Error("No API key provided. Please set TOKKO_API_KEY environment variable.")
    }

    if (!this.security.validateApiKey(this.config.apiKey)) {
  ...
  private async makeSecureRequest<T>(
    ...
    // Validate API key only when making requests
    this.validateApiKey()
...
export function createSecureTokkoClient(apiKey?: string): SecureTokkoClient {
  return new SecureTokkoClient({
    apiKey: apiKey || process.env.TOKKO_API_KEY || "",
