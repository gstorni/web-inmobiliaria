...
// Import after environment variables are verified
...
async function loadTokkoClient() {
  try {
    const tokkoModule = await import("../lib/enhanced-tokko-client")
    console.log("✅ Enhanced Tokko client loaded successfully")
    return tokkoModule.secureTokkoClient
  } catch (error: any) {
    console.error(`❌ Failed to load enhanced Tokko client: ${error.message}`)
    console.error(`   This might be due to invalid TOKKO_API_KEY format`)
    console.error(`   Continuing without Tokko API tests...`)
    return null
  }
}
...
async function investigatePropertyNotFound() {
  ...
  // Load Tokko client safely
  const secureTokkoClient = await loadTokkoClient()
  ...
  for (const propertyId of testPropertyIds) {
    ...
    // Step 5: Test direct Tokko API (only if client loaded successfully)
    if (secureTokkoClient) {
      await checkTokkoAPI(propertyId, results, secureTokkoClient)
    } else {
      console.log(`⚠️ Skipping Tokko API test - client not available`)
      results.push({
        step: `tokko_api_${propertyId}`,
        success: false,
        error: "Tokko client not available due to initialization error",
      })
    }
  ...
  analyzeResults(results, secureTokkoClient)
}
...
async function checkSupabaseCache(propertyId: number, results: PropertyInvestigation[]) {
  ...
  try {
    ...
    const { data, error } = await supabase.from("properties_cache").select("*").eq("tokko_id", propertyId).single()
...
async function checkRedisCache(propertyId: number, results: PropertyInvestigation[]) {
  ...
  try {
    ...
    console.log(`   Redis configuration detected, but direct test not implemented`)
    results.push({
      step: `redis_cache_${propertyId}`,
      success: false,
...
async function checkTokkoAPI(propertyId: number, results: PropertyInvestigation[], secureTokkoClient: any) {
  const startTime = Date.now()
  try {
...
function analyzeResults(results: PropertyInvestigation[], secureTokkoClient: any) {
  console.log(`\n📋 ANALYSIS AND RECOMMENDATIONS`)
  console.log("=".repeat(60))
  ...
  console.log(`\n🔧 NEXT STEPS:`)
  console.log(`1. Fix TOKKO_API_KEY format if Tokko client failed to load`)
  console.log(`2. Run property sync if no properties in cache`)
  console.log(`3. Test with existing property IDs from sample`)
  console.log(`4. Check API endpoint with working property ID`)
  console.log(`5. Verify page routing with valid property`)
  ...
  console.log(`   Tokko client: ${secureTokkoClient ? "✅ Loaded" : "❌ Failed to load"}`)
}
